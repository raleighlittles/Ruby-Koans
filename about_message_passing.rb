...
class AboutMessagePassing < Neo::Koan
  ...
  def test_methods_can_be_invoked_more_dynamically
    ...
    assert mc.send("caught" + "?") # the original verison was missing the question mark

    ##assert mc.send("CAUGHT?".____ )      # What would you need to do to the string?
    assert mc.send("CAUGHT?".downcase) # It should be lowercase instead of uppercase
  end
  ...
  def test_classes_can_be_asked_if_they_know_how_to_respond
    ...
    # MessageCatcher class doesn't have a :does_not_exist method
  end
  ...
  def test_sending_a_message_with_arguments
    ...
    assert_equal [], mc.add_a_payload # No payload was added?
  ...
  def test_catching_messages_makes_respond_to_lie
    ...
    # Catcher doesn't respond to :any_method,
  end
  ...
  def test_explicitly_implementing_respond_to_lets_objects_tell_the_truth
    ...
    # According to the redefined respond_to? method, if the first 3 letters in the method name are "foo" then it'll
    # return true

    ##assert_equal __, catcher.respond_to?(:something_else)
    assert_equal false, catcher.respond_to?(:something_else)
    # The *super* keyword checks up the ancenstry tree to find the inherited method, so in this case it calls
    # the regular "respond_to?" method for Objects, which is why it returns false (since there is no method called
    # 'something else') here
